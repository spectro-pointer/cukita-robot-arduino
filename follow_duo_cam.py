...
gp.setup(21, gp.OUT)
gp.setup(22, gp.OUT)
gp.setup(PIN_CHANGE_DIRECTION, gp.IN)

#
# Setting some gpio pin
#
gp.output(11, True)
gp.output(12, True)
gp.output(15, True)
...
def main():
		...
		cv2.setTrackbarPos(CONTROL_LINE_WIDTH, WINDOW_DISPLAY_IMAGE, SCAN_RADIUS *4)
		
                
		# Start loop
		while True:
			
			if gp.input(PIN_CHANGE_DIRECTION) == True:
				forwardCam()
...
#					print("test")
					...
					#ttyfile =Path ('/dev/ttyACM0')
					cc   = os.system('ls /dev/ttyACM0')
					if cc== 0:
						print ("arduino coenctado") 
						arduino = serial.Serial('/dev/ttyACM0', 9600)
					else:
						print ("arduino desconectado")  #Mandar un comando hacia Arduino
